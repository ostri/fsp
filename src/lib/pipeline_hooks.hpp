...
namespace fsp
{
   ...
   * called by that one thread for the thread's whole lifetime, so on_document_open/close and
   * on_segment_processed never need any locking around a developer's own state. on_run_start()/
   ...
   *
   * All hook bodies default to a no-op (or, for on_segment_processed, a sensible default verdict)
  ...
  public:
    ...
    virtual void on_worker_start([[maybe_unused]] int               worker_id,
                                 [[maybe_unused]] std::string_view  thread_name,
                                 [[maybe_unused]] const fsp_logger& log)
    {
    }
    virtual void on_worker_end([[maybe_unused]] int               worker_id,
                               [[maybe_unused]] std::string_view  thread_name,
                               [[maybe_unused]] const fsp_logger& log)
    {
    }
    ...
    virtual void on_document_open([[maybe_unused]] std::size_t       doc_ndx,
                                  [[maybe_unused]] const doc_dscr&   dscr,
                                  [[maybe_unused]] const fsp_logger& log)
    {
    }
    virtual void on_document_close([[maybe_unused]] std::size_t       doc_ndx,
     ...
     * idea was dropped).
     */
    virtual bool on_segment_processed([[maybe_unused]] std::size_t       seg_id,
   ...
   * Derived had overridden on_segment_processed (via comparing &Derived::on_segment_processed
   * != &pipeline_hooks::on_segment_processed), caching the result in a bool so hot call sites
   ...
   * overridden", silently skipping on_segment_processed for every hook, including ones that did
