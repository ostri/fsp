...
namespace fsp
{
   ...
   * shape long-term): every hook a caller of process_files() is meant to override is now the
   * "_safe" one, declared protected below (on_run_safe_start(), on_wrk_safe_start(), ...), and
   * takes no logger::Logger parameter at all -- call the protected log() accessor instead. The
   * public, non-"_safe" names (on_run_start(), on_wrk_start(), ...) still exist and are still
   * what pipeline/pipeline_worker/xml_worker call, but are now final: a derived class can no
   * longer override them directly, so it can no longer forget to chain to the base
   * implementation the way the OLD on_run_start()/on_wrk_start() required (see this class's
   * git history) to get run_start_/worker_start_/log_ populated -- the final wrapper sets them
   * unconditionally, then dispatches to the "_safe" one, so log() is guaranteed valid inside
   * every "_safe" override without the caller having to do anything for it.
   */
  ...
  public:
     ...
     * and log_ unconditionally, then dispatches to on_run_safe_start() (see the class's own doc
     * comment on why a derived class overrides that one instead of this one).
     */
    virtual void on_run_start(const doc_set_dscr& ds_dscr, const logger::Logger& log) final
    {
      run_start_ = std::chrono::steady_clock::now();
      log_       = &log;
      on_run_safe_start(ds_dscr);
    }
    /**
     ...
     * place all of them are visible at once. Final -- see on_run_start()'s own doc comment.
     */
    virtual void on_run_end(const doc_set_counter&           counters,
                            const doc_set_dscr&              ds_dscr,
                            std::span<const pipeline_hooks*> worker_clones) final
    { on_run_safe_end(counters, ds_dscr, worker_clones); }
     ...
     * on_wrk_safe_start() (see on_run_start()'s own doc comment).
     */
    virtual void on_wrk_start(int worker_id, cstr_t thread_name, const logger::Logger& log) final
    {
      worker_start_ = std::chrono::steady_clock::now();
      log_          = &log;
      on_wrk_safe_start(worker_id, thread_name);
    }
    virtual void on_wrk_end(int worker_id, cstr_t thread_name) final { on_wrk_safe_end(worker_id, thread_name); }

    /** @brief The cutter thread for this specific document (cutting just started/just finished). Final -- see on_run_start()'s own doc
     * comment. */
    virtual void on_doc_open(std::size_t doc_ndx, const doc_dscr& dscr) final { on_doc_safe_open(doc_ndx, dscr); }
    virtual void on_doc_close(std::size_t doc_ndx, doc_status status, const doc_dscr& dscr) final
    { on_doc_safe_close(doc_ndx, status, dscr); }
     ...
     * correct). Final -- see on_run_start()'s own doc comment.
     ...
     * idea was dropped).
     */
    virtual bool on_semantic_check(const xml_segment& segment, segment_result& result, bool is_first, bool is_last) final
    { return on_semantic_safe_check(segment, result, is_first, is_last); }
     ...
     * once for the whole batch. Final -- see on_run_start()'s own doc comment.
     ...
     */
    virtual void on_block_store(std::span<const std::size_t> indices, segment_pool& pool, const doc_set_dscr& ds_dscr) final
    { on_block_safe_store(indices, pool, ds_dscr); }
     ...
     * indices[i] failed). Final -- see on_run_start()'s own doc comment.
     */
    virtual void on_failed_block_store(std::span<const std::size_t> indices,
                                       std::span<const error_info>  errors,
                                       segment_pool&                pool,
                                       const doc_set_dscr&          ds_dscr) final
    { on_failed_block_safe_store(indices, errors, pool, ds_dscr); }
  protected:
    /**
     * @brief This run's logger -- valid inside every "_safe" hook below (on_run_start()/
     ...
     * dispatching to any other "_safe" hook). Asserts rather than dereferencing null if somehow
     * called before either of those, e.g. from a derived class's own constructor.
     */
    [[nodiscard]] const logger::Logger& log() const
    {
      assert(log_ != nullptr && "pipeline_hooks::log() called before on_run_start()/on_wrk_start() -- see their own doc comments");
      return *log_;
    }

    /// @brief Override point for on_run_start() -- see the class's own doc comment. log() is valid inside this call.
    virtual void on_run_safe_start([[maybe_unused]] const doc_set_dscr& ds_dscr) { }
    /// @brief Override point for on_run_end() -- see the class's own doc comment. log() is valid inside this call.
    virtual void on_run_safe_end([[maybe_unused]] const doc_set_counter&           counters,
                                 [[maybe_unused]] const doc_set_dscr&              ds_dscr,
                                 [[maybe_unused]] std::span<const pipeline_hooks*> worker_clones)
    {
    }
    /// @brief Override point for on_wrk_start() -- see the class's own doc comment. log() is valid inside this call.
    virtual void on_wrk_safe_start([[maybe_unused]] int worker_id, [[maybe_unused]] cstr_t thread_name) { }
    /// @brief Override point for on_wrk_end() -- see the class's own doc comment. log() is valid inside this call.
    virtual void on_wrk_safe_end([[maybe_unused]] int worker_id, [[maybe_unused]] cstr_t thread_name) { }
    /// @brief Override point for on_doc_open() -- see the class's own doc comment. log() is valid inside this call.
    virtual void on_doc_safe_open([[maybe_unused]] std::size_t doc_ndx, [[maybe_unused]] const doc_dscr& dscr) { }
    /// @brief Override point for on_doc_close() -- see the class's own doc comment. log() is valid inside this call.
    virtual void on_doc_safe_close([[maybe_unused]] std::size_t     doc_ndx,
                                   [[maybe_unused]] doc_status      status,
                                   [[maybe_unused]] const doc_dscr& dscr)
    {
    }
    /**
     * @brief Override point for on_semantic_check() -- see the class's own doc comment. log() is
     ...
     */
    virtual bool on_semantic_safe_check([[maybe_unused]] const xml_segment& segment,
                                        segment_result&                     result,
                                        [[maybe_unused]] bool               is_first,
                                        [[maybe_unused]] bool               is_last)
    { return result.values().complete(); }
    /// @brief Override point for on_block_store() -- see the class's own doc comment. log() is valid inside this call.
    virtual void on_block_safe_store([[maybe_unused]] std::span<const std::size_t> indices,
                                     [[maybe_unused]] segment_pool&                pool,
                                     [[maybe_unused]] const doc_set_dscr&          ds_dscr)
    {
    }
    /// @brief Override point for on_failed_block_store() -- see the class's own doc comment. log() is valid inside this call.
    virtual void on_failed_block_safe_store([[maybe_unused]] std::span<const std::size_t> indices,
  ...
  private:
    ...
    // Not owned -- points at whichever logger::Logger on_run_start()/on_wrk_start() was last
    // called with (the original instance's own, or this clone's own worker thread logger
