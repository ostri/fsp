...
namespace fsp
{
  ...
  constexpr bool has_on_type_for_every_v =
    (... && requires(Derived& d, Ts& s, segment_result& r, bool b, const logger::Logger& log) { d.on_type(s, r, b, b, log); });
   ...
   *                bool is_first, bool is_last, const logger::Logger& log);
   *
   * -- with the exact same parameter list pipeline_hooks::on_semantic_check() itself takes,
   * minus the raw xml_segment (materialize_variant() already replaces it with s) and with the
   * segment's own materialized type as the first parameter instead. on_semantic_check() itself
   * is implemented here, once, and marked final -- a derived class overriding it instead of
   * declaring on_type() overloads would silently lose this whole mechanism, so that path is
   * closed off entirely rather than left as a footgun.
   *
   * @details on_semantic_check()'s own static_assert requires Derived to declare an on_type()
   * overload for EVERY schema class Namespace declares, not just the ones a caller happens to
   * remember -- deliberately a compile-time error over a silent runtime fallback (e.g. a warning
   * logged the first time an unhandled type is seen): a missing or misspelled overload is a
   * caller mistake that pipeline_hooks::on_semantic_check()'s own default body would otherwise
   * paper over by returning result.values().complete() unconditionally, exactly like every OTHER
   * schema class silently would if this mixin instead defaulted to "ok" for a type with no
   * overload. If a schema class genuinely has no business rule of its own, its on_type()
   * overload still needs to exist -- just with a body that returns true unconditionally -- so
   * that decision is visible in the derived class's own source instead of being inferred from an
   * absence. The check itself lives in on_semantic_check(), not typed_semantic_check's own class
   * body: a class template's member functions are only instantiated on first use, well after
   * Derived (e.g. pacs8_cb) is a complete type, whereas the base class itself is instantiated
   * while Derived is still being defined (CRTP's usual "incomplete type" trap) -- a static_assert
   * placed directly in the class body would see NO on_type() overloads yet, no matter how many
   * Derived actually declares, and always fail.
  ...
  {
  public:
    bool on_semantic_check([[maybe_unused]] const xml_segment& segment,
                           segment_result&                     result,
                           bool                                is_first,
                           bool                                is_last,
                           const logger::Logger&               log) final
    {
      ...
      static_assert(check_on_type_coverage<Derived>(static_cast<variant_of_t<Namespace, Base>*>(nullptr)),
                    ...
                    "bool on_type(const YourSchemaClass&, segment_result&, bool, bool, const logger::Logger&) for each one.");
      auto seg = materialize_variant<Namespace, Base>(result.seg_type(), result);
      return std::visit([&]<typename T>(T& s) -> bool { return static_cast<Derived&>(*this).on_type(s, result, is_first, is_last, log); },
                        seg);
    }
  };
} // namespace fsp
