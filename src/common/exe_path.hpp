...
namespace fsp
{
   ...
   * files that ship next to its own binary (e.g. config/log.<debug|release>.json, copied there by
   * add_log_config() in CMakeLists.txt) regardless of the caller's current working directory.
   *
