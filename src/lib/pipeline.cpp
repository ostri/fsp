...
namespace fsp
{
  ...
  void pipeline::record_segment_done(std::size_t doc_ndx, bool semantically_ok)
  {
    if ((*doc_counters_)[doc_ndx].record_segment(semantically_ok)) log_doc_done(doc_ndx);
  }

  // Called once a document is complete -- whichever of record_doc_close()/record_segment_done()
  // turns out to be the one that satisfies the last remaining condition (see
  // doc_counters::maybe_complete()) is the one whose call returns true and triggers this.
  void pipeline::log_doc_done(std::size_t doc_ndx)
  {
    const auto& c = (*doc_counters_)[doc_ndx];
    log_.info(fmt::format("Doc {}: cut+process finished ({} segments, {} ms).", doc_ndx, c.total(), c.processing_doc().count()));
  }
